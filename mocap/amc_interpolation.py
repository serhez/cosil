...
def interpolate(amc_file, output_file, src_framerate, dst_framerate, use_pchip=False):
    root = np.empty((0, 6))
    ...
    for i in range(0, len(joints)):
        ...
        if use_pchip:
            for dim in range(joints[i].shape[1]):
                pc = PchipInterpolator(frames, joints[i][:, dim])
                jointsn[:, dim] = pc(redFrames)
