...
class SAC(object):
    def __init__(self, num_inputs: int, action_space, num_morpho_obs: int, num_morpho_parameters: int, args):
    ...
    # Only used for pretraining SAIL
    def pretrain_policy(self, rewarder, memory: ReplayMemory, batch_size: int, n_epochs=200):
    ...
    def pretrain_value(self, rewarder, memory: ReplayMemory, batch_size: int):
        for i in range(3000):
    ...
    def update_parameters(self, batch, rewarder, updates: int, update_value_only=False):
        state_batch, action_batch, reward_batch, next_state_batch, mask_batch, marker_batch, next_marker_batch = batch
        ...
        if rewarder is not None:
            new_rewards = rewarder.compute_rewards(marker_feats)
        else:
