...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, parametric=True, init_task=None, limit_joints=True):
        self.original_lengths = np.array([1., .15, .145, .094, .133, .07])
        ...
        self.markers = ["thigh", "foot", "foottip"]
        self.legs = ["b", "f"]
        self.origin = "torso"

        MujocoEnv.__init__(self, '2seg_half_cheetah.xml', 5)
    ...
    def limb_segment_lengths(self):
        return self.current_lengths[2:].reshape(2, 2)*2

    @property
    def morpho_params(self):
        val = self.limb_segment_lengths.flatten() / 2
        assert val.shape == (4, )
        return val
...
gym.envs.register(
        id="GaitTrack2segHalfCheetah-v0",
...
gym.envs.register(
        id="GaitTrack2segHalfCheetahUnlimited-v0",
        entry_point="%s:HalfCheetahEnv" % __name__,
        max_episode_steps=1000,
        kwargs={"parametric": True, "limit_joints": False},
)
